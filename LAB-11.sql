...
--3. Display the current date in a format that appears as may 5 1994 12:00AM. 
	...
	SELECT FORMAT(GETDATE(),'MMM dd yyyy h:m')
...
--11. Extract month from current date using datename () and datepart () function.
	...
	SELECT DATENAME(MM,GETDATE()) AS date_name ,DATEPART(MM,GETDATE()) AS date_part 
...
--13. Calculate your age in years and months. 
	...
	SELECT DATEDIFF(yy,'2008-03-15',GETDATE()),DATEDIFF(mm,'2008-03-15',GETDATE())
...
--21. Display number of accounts opened in each year. 
	...
	SELECT COUNT(*),YEAR(ADATE) FROM DEPOSIT
	GROUP BY YEAR(ADATE)

--22. Display number of accounts opened in each month. 

	SELECT COUNT(*),MONTH(ADATE) FROM DEPOSIT
...
--28. Display difference in days between today’s date and account date.
	...
	SELECT DATEDIFF(DD,ADATE,GETDATE()) FROM DEPOSIT
